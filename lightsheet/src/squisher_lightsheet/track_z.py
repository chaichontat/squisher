...
def robust_normalize(image: np.ndarray) -> np.ndarray:
    ...
    centered = clipped - float(np.median(clipped[np.isfinite(clipped)]))
    denom = max(float(np.percentile(np.abs(centered[np.isfinite(centered)]), 95.0)), 1.0)
    out[np.isfinite(clipped)] = centered[np.isfinite(clipped)] / denom
    return out
