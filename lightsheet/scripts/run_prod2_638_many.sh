...
OUTPUT_DIR="${RUN_ROOT}/xreg-638-many"
LOG_PATH="${RUN_ROOT}/run-logs/xreg-638-many.log"
SWEEP="/home/chaichontat/nvme/lightsheet/scripts/run_fused_fixed_method8_sweep.py"
WINDOWS="/home/chaichontat/squisher/lightsheet/scripts/prod2_638_many_windows.json"

# Acquisition source channel 1 (638) is fitted to the completed source-channel-0
# 561 mosaic. Two spatially alternating windows are sampled from every tile.
MOVING_CHANNEL=1
...
echo "638->561 many-tile Method 6 run"
echo "python=${EXPECTED_PYTHON}"
echo "moving_channel=${MOVING_CHANNEL}"
echo "windows=$(jq length "${WINDOWS}")"
echo "gpu_0=${GPU_5090}"
echo "gpu_1=${GPU_4090}"
